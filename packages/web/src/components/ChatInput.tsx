...
const SKILL_OPTIONS = ['文档处理', '视频生成', '深度研究', '幻灯片', '数据分析', '数据可视化', '金融服务'] as const;
...
export function ChatInput({
  ...
  const filteredSkillOptions = useMemo(() => {
    ...
    if (!lower) return SKILL_OPTIONS;
    return SKILL_OPTIONS.filter((name) => name.toLowerCase().includes(lower));
  }, [skillFilter]);
      ...
      <div className="p-4 pt-2">
        ...
        <div className="flex-1">
          <div className="mx-auto w-[80%]">
            ...
            <div
              className={`relative overflow-hidden rounded-2xl border bg-white focus-within:ring-2 ${
                whisperMode
            ...
            >
              <div className="relative">
                ...
                <textarea
                  ...
                  className="block min-h-[114px] w-full resize-none bg-transparent p-3 text-sm placeholder:text-gray-400 focus:outline-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0"
                  style={{
                    textIndent: selectedMentions.length > 0 ? `calc(${mentionIndentPx}px + 2ch)` : undefined,
                  }}
                  rows={1}
                  disabled={disabled}
                >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="relative">
                    ...
                    {showSkillMenu && (
                      <div
                        ref={menuRef}
                        className="absolute bottom-full left-0 mb-2 z-20 flex w-[200px] flex-col overflow-hidden rounded-xl border border-gray-200 bg-white p-2 shadow-lg"
                      >
                        ...
                        <div className="max-h-[220px] overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0">
                          {filteredSkillOptions.map((skill, i) => (
                            <button
                              key={skill}
                              type="button"
                              className={`flex h-[34px] w-full items-center rounded-[6px] p-2 text-left text-[12px] font-normal text-[#191919] transition-colors ${
                                i === selectedIdx ? 'bg-[rgba(240,247,255,1)]' : 'hover:bg-[rgba(240,247,255,0.1)]'
                              }`}
                        ...
                        <button
                          type="button"
                          className="mt-2 h-[34px] rounded-[6px] border border-[rgba(219,219,219,0.8)] text-[12px] text-[#191919] transition-colors hover:bg-gray-50"
                          onMouseDown={(e) => {
                            e.preventDefault();
                            closeMenus();
